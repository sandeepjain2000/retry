...
def find_and_click_retry():
    ...
    print("\n🔔 Checking in 15 seconds...", end="", flush=True)
    ...
    # Strategy 3: Check for "Please review the implementation plan" text
    review_phrase = "please review the implementation plan"

    # Check from word-level OCR data already collected above
    full_ocr_words = " ".join([w for w in words if w.strip()])
    if review_phrase in full_ocr_words:
        print(f"✅ Found 'Please review the implementation plan' — typing GO AHEAD and pressing Enter")
        time.sleep(0.5)
        pyautogui.typewrite("GO AHEAD", interval=0.05)
        time.sleep(0.2)
        pyautogui.press("enter")
        return "review"

    # Fallback: line-aware OCR (catches phrase split across lines)
    full_ocr_text = pytesseract.image_to_string(gray, config='--oem 3 --psm 6').strip().lower()
    if review_phrase in full_ocr_text:
        print(f"✅ Found 'Please review the implementation plan' (line OCR) — typing GO AHEAD and pressing Enter")
        time.sleep(0.5)
        pyautogui.typewrite("GO AHEAD", interval=0.05)
        time.sleep(0.2)
        pyautogui.press("enter")
...
try:
    while True:
        ...
        if clicked:
            if clicked == "retry":
                print("✅ Retry clicked, waiting 120 seconds...")
                time.sleep(120.0)
            elif clicked == "review":
                print("✅ Typed GO AHEAD, waiting 120 seconds...")
                time.sleep(120.0)
            else:
                print(f"✅ '{clicked}' clicked, waiting 15 seconds...")
                time.sleep(15.0)
        else:
